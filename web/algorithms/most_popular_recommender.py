...
def most_popular_recommender(
    beh_df, news_df, category_filter=[], recommender=False
):
    def find_index(series, name):
        try:
            index = (
                series.index.get_loc(name) + 1
    ...
    for index, row in beh_df.iterrows():
        ...
        if recommender:
            filter_df = beh_df[
                (pd.to_datetime(beh_df["Time"], format=format) >= start_date)
        ...
        for ids in split_ids:
            all_article_ids.extend(
                [id.split("-")[0] for id in ids if id.endswith("-1")]
            )

        # Filter out articles not in wanted subcategories
        if len(category_filter) != 0:
            for article_id in all_article_ids:
                art_subcat = news_df[news_df["ID"] == article_id][
                    "Subcategory"
                ].values[0]
        ...
        split_Impression_list = [
            word for sublist in impression_ids for word in sublist.split()
        ]
