...
def draw_graph_sim_subcat(news_df):
    ...
    colors = [
        '#1f77b4', '#8c564b', '#98df8a', '#c7c7c7', '#aec7e8',
        '#bcbd22', '#c49c94', '#d62728', '#dbdb8d', '#e377c2',
        '#ff7f0e', '#ff9896', '#17becf', '#9467bd', '#f7b6d2',
        '#ffbb78', '#2ca02c', '#7f7f7f', '#9edae5', '#c5b0d5'
    ]

    # Assign colors to categories dynamically
    category_color_map = {category: colors[i % len(colors)] for i, category in enumerate(categories)}
    ...
    for category in categories:
        ...
        subcategories = news_df[news_df['Category'] == category]['Subcategory'].unique()
        shared_subcategories = find_shared_subcats(news_df)
        print(category)
        for i in range(len(subcategories)):
            subcat1 = subcategories[i] if not subcategories[i] in shared_subcategories else f'{category}_{subcategories[i]}'
            G.add_node(subcat1, node_type='subcategory', color=category_color_map[category])
            for j in range(i + 1, len(subcategories)):
                subcat2 = subcategories[j] if not subcategories[j] in shared_subcategories else f'{category}_{subcategories[j]}'
    ...
    pos = nx.spring_layout(G, k=1/math.sqrt(math.sqrt(number_of_nodes)))  # Layout algorithm for graph visualization
    ...
    plt.title('Subcategory Similarity Graph within Main Categories', fontsize=30)
    plt.tight_layout(pad=0.5)
    plt.axis('off')
...
def compute_sims_subcat(news_df):
    ...
    for category in categories:
        ...
        for subcategory in subcategories:
            titles = category_news[category_news['Subcategory'] == subcategory]['Title'].tolist()
            preprocessed_titles = [preprocess(title) for title in titles]
        ...
        for subcat1 in subcategories:
            ...
            filtered_results = {pair: similarity for pair, similarity in results.items() if pair[0] == subcat1}
...
def sim_cat_rec(beh_df, news_df, user_id='U81540'):
    ...
    top_cats, top_subcats = get_top_categories(beh_df_user, news_df, user_id, count=3)

    # Get similiar categories to favorite catgories
    fav_cats = []

    for subcat in top_subcats:
        subcat_cat = news_df[news_df['Subcategory'] == subcat]['Category'].unique()[0]
